...
const restoreDocumentWithSubcollection = functions.https.onCall(async (data, context) => {
    ...
    if (!docSnapshot.exists) {
        throw new functions.https.HttpsError('not-found', 'Document to restore does not exist in recycle bin.');
    }
    ...
    if (!checkSnapshot.empty) {
        ...
        const restorePromises = checkSnapshot.docs.map(doc => {
            ...
            const originalLocationRef = originalDocRef.collection('checks').doc(doc.id);
    ...
    await Promise.all(checkSnapshot.docs.map(doc => checkSubcollectionRef.doc(doc.id).delete()));
