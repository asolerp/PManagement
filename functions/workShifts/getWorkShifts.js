/**
 * Callable functions (v2) for the admin dashboard.
 * Migrated from v1 to fix CORS preflight failures from web clients.
 */
 ...
 */
exports.getWorkShifts = onCall(CALLABLE_OPTS, async request => {
  try {
    if (!request.auth) {
 ...
 */
exports.getWorkShiftStats = onCall(CALLABLE_OPTS, async request => {
  try {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated.');
    }
 ...
 */
exports.getWorkers = onCall(CALLABLE_OPTS, async request => {
  try {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated.');
    }
    ...
    snapshot.forEach(doc => {
      ...
      if (photo && photo.includes('cloudinary')) photo = null;
