/**
 * Tool: obtiene el detalle de una incidencia por ID.
 */
...
async function run(companyId, args) {
  ...
  const ref = db.collection('incidences').doc(incidentId);
  ...
  if (data.houseId) {
    const houseSnap = await db.collection('houses').doc(data.houseId).get();
