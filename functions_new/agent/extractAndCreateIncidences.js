/**
 * F2: A partir de una transcripción de voz, extrae propiedad e incidencias con el LLM
 * y crea las incidencias (una por ítem) usando la tool createIncidence.
 */
...
/**
 * Extrae propiedad e incidencias de la transcripción usando el LLM.
 * @param {string} apiKey
 * @param {string} transcription
 * @returns {Promise<{ propertyName: string, incidents: Array<{ title: string, description?: string }> }>}
 */
...
/**
 * Crea las incidencias en Firestore usando la tool createIncidence.
 * Si la propiedad no está clara o hay varias coincidencias, el primer resultado de la tool se devuelve.
 ...
 * @param {string[]} [photoUrls] - URLs de fotos de la sesión para adjuntar a cada incidencia
 * @returns {{ created: number, message: string, needConfirm?: string }}
 */
async function createIncidencesFromExtraction(
  ...
  if (created === 0) {
    return {
      created: 0,
      message:
        'No se pudo crear ninguna incidencia. Comprueba que el nombre de la propiedad exista en tu empresa (usa "lista de propiedades" en el bot).',
    };
  }
  return {
    created,
    message: `Se han creado ${created} incidencia${created !== 1 ? "s" : ""} en "${propertyName}".`,
  };
}
