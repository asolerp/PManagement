...
exports.proposeQuadrantAssignment = onCall(
  ...
  async (request) => {
    ...
    /** Máximo de casas que caben en la ventana del trabajador (duración mínima + buffer entre casas). */
    ...
    // Cargar propiedades (casas) con companyId y opcional location
    const housesMap = {};
    const coordsMap = {};
    for (const hid of uniqueHouseIds) {
      const snap = await db.collection("properties").doc(hid).get();
      if (!snap.exists) continue;
      const data = snap.data();
      if (data.companyId && data.companyId !== companyId) continue;
      housesMap[hid] = {
        id: hid,
    ...
    // Cargar trabajadores (users) con companyId y opcional homeLocation
    const workersMap = {};
    const workerOrigins = {};
    ...
    if (validHouseIds.length === 0 || validWorkerIds.length === 0) {
      return {
        assignments: [],
        message:
          "No se encontraron casas o trabajadores válidos para tu empresa.",
      };
    }

    // Límite de casas por trabajador según su ventana horaria
    const maxHousesPerWorker = {};
    validWorkerIds.forEach((wid) => {
      maxHousesPerWorker[wid] = Math.max(1, getMaxHousesForWorker(wid));
    });

    // Asignar cada casa a un trabajador (respetando capacidad por ventana)
    ...
    if (hasCoords) {
      ...
      validHouseIds.forEach((houseId, i) => {
        const wid = chooseWorkerForHouse(houseId);
        workerHouseIds[wid].push(houseId);
      });
    }

    // Para cada trabajador: ordenar sus casas (nearest-neighbor si hay coords) y asignar franjas
    // Duración por casa: houseDurations[houseId] ?? defaultDurationMinutes
    const assignments = [];
