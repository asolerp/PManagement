...
async function generateMonthlyAndAnnualReports(
  ...
  try {
    ...
    let monthlyQuery = admin.firestore().collection("timeEntries");

    if (companyId) {
      monthlyQuery = monthlyQuery.where("companyId", "==", companyId);
    }
    if (workerId) {
    ...
    let annualQuery = admin.firestore().collection("timeEntries");

    if (companyId) {
      annualQuery = annualQuery.where("companyId", "==", companyId);
    }
    if (workerId) {
      annualQuery = annualQuery.where("worker.id", "==", workerId);
    }
    annualQuery = annualQuery
