...
export default function Modal({ open, onClose, title, children, maxWidth = 'max-w-lg' }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative w-full ${maxWidth} bg-[var(--surface-elevated)] rounded-2xl shadow-xl border border-[var(--border-soft)] animate-modal-in`}
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--border-soft)]">
          ...
          <button
            onClick={onClose}
            className="p-2.5 -mr-2 rounded-xl hover:bg-stone-100 transition-colors text-stone-500"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
