...
function CompanyProfileSection() {
  ...
  const maxHouses = company.maxHouses ?? 20;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-4 sm:p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Plan y límites</h2>
        <p className="text-sm text-gray-500 mb-4">
          Límites de tu plan actual. Para ampliarlos, contacta con nosotros para cambiar de plan.
        </p>
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl min-w-[140px]">
            <div className="p-2 rounded-lg bg-[#126D9B]/10">
              <Users className="w-5 h-5 text-[#126D9B]" />
            </div>
            <div>
              <p className="text-sm text-gray-500">Usuarios</p>
              <p className={`font-semibold ${users.length >= maxUsers ? 'text-amber-600' : 'text-gray-900'}`}>
                {users.length} / {maxUsers}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl min-w-[140px]">
            <div className="p-2 rounded-lg bg-[#3B8D7A]/10">
              <Home className="w-5 h-5 text-[#3B8D7A]" />
            </div>
            <div>
              <p className="text-sm text-gray-500">Propiedades</p>
              <p className={`font-semibold ${properties.length >= maxHouses ? 'text-amber-600' : 'text-gray-900'}`}>
                {properties.length} / {maxHouses}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl">
            <span className="px-3 py-1 rounded-full text-sm font-medium bg-amber-100 text-amber-800 capitalize">
              Plan {plan}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm sm:text-base text-gray-500 hidden sm:block mt-1">
            Datos de la empresa, plan y catálogos
          </p>
        </div>
        {activeTab !== 'empresa' && (
